...
from cloud_removal.models.models_utils import weights_init, print_network
from cloud_removal.models.layers import *
...
###### ✅ Safe Generator for Streamlit / CPU / GPU environments
class Generator(nn.Module):
    def __init__(self, gpu_ids):
        super().__init__()
        # Ensure gpu_ids is always a list
        self.gpu_ids = gpu_ids if isinstance(gpu_ids, list) else []

        # Create the SPANet generator
        self.gen = nn.Sequential(OrderedDict([('gen', SPANet())]))
        self.gen.apply(weights_init)

    def forward(self, x):
        # Use GPU only if available and gpu_ids list is valid
